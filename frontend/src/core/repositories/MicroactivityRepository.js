...
export class MicroactivityRepository {
  ...
  async findByCategory(category) {
    try {
      ...
      throw this.handleError(error, 'Error al obtener microactividades por categor�a');
    }
  }
  async search(searchTerm) {
    try {
      ...
      throw this.handleError(error, 'Error en b�squeda de microactividades');
    }
  }
  async getStats() {
    try {
  ...
  validateId(id) {
    ...
    if (typeof id === 'string' && isNaN(Number(id))) {
      throw new Error('ID debe ser num�rico');
    }
  }
  ...
  handleError(error, defaultMessage) {
    ...
    if (error.response) {
      ...
      if (status === 401) {
        return new Error('No autorizado. Por favor inicia sesi�n.');
      } else if (status === 403) {
