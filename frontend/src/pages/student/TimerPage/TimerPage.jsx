...
import ConfirmationModal from 'components/features/modals/ConfirmationModal';
import EndModal from 'components/features/modals/EndModal';
import DurationConfigModal from 'components/features/modals/DurationConfigModal';
...
function TimerPage() {
  ...
  useEffect(() => {
    microactivityTimerService.getAllForTimer().then(setAllMicroactivities);
  }, []);
  ...
  const handleReset = useCallback(async () => {
    try {
      ...
      setError(null);
      // Espera breve para asegurar que el estado se actualice antes de iniciar
      setTimeout(() => {
      ...
      <div className="timer-page__minimalist-container">
        ...
        <div className="timer-page__next-activity-card">
          <div className="timer-page__card-header">
            ...
            <div className="timer-page__dropdown-wrapper">
              <button className="timer-page__dropdown-btn" onClick={() => setShowDropdown(v => !v)}>
                <span role="img" aria-label="Elegir">🔽</span>
              </button>
              {showDropdown && (
                <div className="timer-page__dropdown-list">
                  {allMicroactivities.map(activity => (
                    <div key={activity.id} className="timer-page__dropdown-item" onClick={() => { setNextMicroactivity(activity); setShowDropdown(false); }}>
                      <span className="timer-page__dropdown-emoji">{getCategoryEmoji(activity.category)}</span>
                      <span className="timer-page__dropdown-title">{activity.title}</span>
                      <span className="timer-page__dropdown-duration">{activity.durationMinutes} min</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="timer-page__card-content">
            <div className="timer-page__activity-category">
              <div className="timer-page__category-icon">
                {getCategoryEmoji(nextMicroactivity?.category)}
              </div>
          ...
          <div className="timer-page__card-footer">
            <p>Esta microactividad se podrá iniciar cuando termine el timer.</p>
          </div>
        </div>
        {}
        {error && (
